...
class Workflow extends retryable_execution_1.default {
    ...
    buildBrowserOptions() {
        if (process.env.BROWSERLESS_API_KEY) {
            return {
                executablePath:require("puppeteer").executablePath(),headless:true,args:["--no-sandbox","--disable-setuid-sandbox","--disable-dev-shm-usage","--proxy-server=socks5://"+process.env.PROXY_USERNAME+":"+process.env.PROXY_PASSWORD+"@"+process.env.PROXY_SERVER],
            };
        }
        return {
            headless: false,
